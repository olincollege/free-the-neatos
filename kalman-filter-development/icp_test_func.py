...
class Align2D:
	...
	def FindCorrespondences(self,src_pts):
		...
		# build arrays of nearest neighbor target points and corresponding source points
		valid_src_pts = []
		valid_tgt_pts = []
		for i, idx_val in enumerate(indices):
			if idx_val != -1:
				valid_tgt_pts.append(self.target[idx_val, :2])
				valid_src_pts.append(src_pts[i, :2])

		matched_src = np.array(valid_src_pts)
		matched_tgt = np.array(valid_tgt_pts)

		return matched_tgt, matched_src, indices
