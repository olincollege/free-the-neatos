...
from freato.angle_helpers import euler_from_quaternion, quaternion_from_euler
...
def stamped_transform_to_pose(t):
    t = t.transform
    return Pose(
        position=Point(x=t.translation.x, y=t.translation.y, z=t.translation.z),
...
class TFHelper(object):
    ...
    def fix_map_to_odom_transform(self, robot_pose, odom_pose):
        ...
        odom_pose_frame = PyKDL.Frame(
            V=PyKDL.Vector(
                x=odom_pose.position.x, y=odom_pose.position.y, z=odom_pose.position.z
            ),
    ...
    def convert_scan_to_polar_in_robot_frame(self, msg, base_frame):
        ...
        laser_pose = stamped_transform_to_pose(
            self.tf_buffer.lookup_transform(base_frame, msg.header.frame_id, Time())
        )
        rot = PyKDL.Rotation.Quaternion(
            x=laser_pose.orientation.x,
        ...
        return (
            msg.ranges,
            np.linspace(
                msg.angle_min + laser_yaw, msg.angle_max + laser_yaw, len(msg.ranges)
            ),
        )
